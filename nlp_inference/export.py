...
import numpy as np
import os
import nlp_inference.main as main
import tqdm

from copy import copy
from nlp_inference.decorators import with_caching
from nlp_inference.texts import split_in_sentences
...
class SNLI(object):
    ...
    @with_caching('SNLI_predict_batch')
    def predict_batch(self, request):
        ...
        SNLI.validate_request(copy(request))

        data, batch_size = SNLI.transform_input(request)
    ...
    def __str__(self):
        return "[ SNLI model " + os.path.basename(self.weights_file) + " ]"
