...
from multinli.arch import define_model
from multinli.utils import read_data_as_json, print_data_stats
from multinli.utils import remove_unlabeled_data, all_sentences
from multinli.utils import pick_best_model_from_dir
from multinli.settings import TRAIN_DATA_PATH
from multinli.settings import VALIDATE_DATA_PATH
from multinli.settings import TEST_DATA_PATH
from multinli.settings import DEBUG, PREMISE_LEN, HYPOTHESIS_LEN
from multinli.settings import WORD_EMBEDDINGS_PATH, WORD_EMBEDDINGS_DIM
from multinli.word_embeddings_loader import WordEmbeddings
...
def predict_batch(test_data, weights_file, verbose=False):
    ...
    for i in range(0, num_tests):
        predicted = y[i]
        res.append(1.0 - np.dot(predicted, [0.0, 1.0, 2.0]) / 2.0)
