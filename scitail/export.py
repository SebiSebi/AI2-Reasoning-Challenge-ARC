...
import numpy as np
import os
import scitail.main as main
import tqdm

from copy import copy
from scitail.decorators import with_caching
from scitail.texts import split_in_sentences
...
class SciTail(object):
    ...
    @with_caching('SciTail_predict_batch')
    def predict_batch(self, request):
        ...
        SciTail.validate_request(copy(request))

        data, batch_size = SciTail.transform_input(request)
    ...
    def __str__(self):
        return "[ SciTail model " + os.path.basename(self.weights_file) + " ]"
